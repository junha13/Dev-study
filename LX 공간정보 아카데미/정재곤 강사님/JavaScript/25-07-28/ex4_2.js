...
router.route(`/page/insert`).get(async(req, res) => {
    ...
    try {
        ...
        let rows = await conn.query(sql, []); // 빈 배열 넣기
...
router.route(`/page/plus`).get(async(req, res) => {
    console.log(`plus 요청됨`);
    const params = req.query;
...
router.route(`/page/update`).get(async(req, res) => {
    ...
    let sql = `Update test.person set name = '${params.name}', age=${params.age}, mobile = '${params.mobile}' where id = ${params.index}`
